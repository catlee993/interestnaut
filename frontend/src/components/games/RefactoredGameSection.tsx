...
import { MediaSectionLayout, MediaGrid } from "@/components/common/MediaSectionLayout";
...
export const RefactoredGameSection = forwardRef<GameSectionHandle, {}>((props, ref) => {
  ...
  const mapGameToSuggestionItem = useCallback((game: GameItem): MediaSuggestionItem => {
    return {
      id: game.id,
      title: game.name,
  ...
  const renderGamePoster = useCallback((item: MediaSuggestionItem) => {
  ...
  ), [mediaSection.searchResults, mediaSection.handleSave, mediaSection.handleAddToWatchlist]);
  ...
  ), [mediaSection.watchlistItems, mediaSection.handleSave, mediaSection.handleRemoveFromWatchlist]);

  const renderSavedItems = useCallback(() => (
    <MediaGrid>
      {mediaSection.savedItems.map((game, index) => (
        <Box 
          key={`saved-${game.id || index}`}
          sx={{ cursor: 'pointer' }}
        >
          <GameCard
            game={game}
  ...
  ), [mediaSection.savedItems, mediaSection.handleSave, mediaSection.handleAddToWatchlist]);
  ...
  return (
    <MediaSectionLayout
      ...
      onLikeSuggestion={() => {
        ...
        if (mediaSection.suggestedItem) {
          ...
          if (!mediaSection.savedItems.some(g => g.id === updatedGame.id)) {
            mediaSection.setSavedItems([...mediaSection.savedItems, updatedGame]);
          }
        }
      }}
      onDislikeSuggestion={() => mediaSection.handleFeedback(session.Outcome.disliked)}
      onSkipSuggestion={() => {
      ...
      onToggleWatchlist={() => mediaSection.setShowWatchlist(!mediaSection.showWatchlist)}
      onToggleLibrary={() => mediaSection.setShowLibrary(!mediaSection.showLibrary)}
