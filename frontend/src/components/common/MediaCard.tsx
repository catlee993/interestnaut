...
export interface MediaItem {
  ...
  vote_average: number;
  vote_count: number;
  date?: string; // release_date for movies, first_air_date for shows
  isSaved?: boolean;
  director?: string;
  writer?: string;
  mediaType: 'movie' | 'tv';
...
export function MediaCard({
  ...
  const MediaIcon = isMovie ? Movie : Tv;
    ...
    <StyledCard>
      <Box sx={{ position: "relative", width: "100%", paddingTop: "150%" }}>
        {hasPoster ? (
          ...
          >
            <MediaIcon sx={{ fontSize: 60, opacity: 0.7, mb: 2 }} />
        ...
        <Overlay>
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center" }}>
              ...
              {item.vote_average > 0 && (
                <Tooltip title={`${item.vote_count} votes`}>
                  <Chip
                    label={item.vote_average.toFixed(1)}
                    size="small"
                    sx={{
                      bgcolor: getScoreColor(item.vote_average),
                      color: "white",
                      height: "20px",
                      "& .MuiChip-label": { px: 1 },
                    }}
