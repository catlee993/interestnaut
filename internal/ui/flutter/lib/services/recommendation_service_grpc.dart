...
class GrpcRecommendationService extends ChangeNotifier {
  static final GrpcRecommendationService _instance = GrpcRecommendationService._internal();
  factory GrpcRecommendationService() => _instance;
  GrpcRecommendationService._internal();
  ...
  Future<Map<String, dynamic>> _gatherUserBehaviorData(String mediaType) async {
    try {
      ...
      // Extract vector media IDs for gRPC calls
      final likedMediaIds = likedSuggestions.map((s) => s.mediaId).toList();
      final dislikedMediaIds = dislikedSuggestions.map((s) => s.mediaId).toList();
      final favoritedMediaIds = favoriteSuggestions.map((s) => s.mediaId).toList();
      final watchlistMediaIds = watchlistSuggestions.map((s) => s.mediaId).toList();
      final skippedMediaIds = skippedSuggestions.map((s) => s.mediaId).toList();
      final excludeMediaIds = allExistingSuggestions.map((s) => s.mediaId).toList();
