...
class LlamaService {
  ...
  Future<bool> initialize(String modelPath, {ModelParams? modelParams, Function(String message, {bool isError})? toastCallback}) async {
    ...
    try {
      debugPrint('🔧 Initializing LLM with model: ${path.basename(modelPath)}');
      debugPrint('📍 Full model path: $modelPath');

      // Check if model exists first
      final modelFile = File(modelPath);
      if (!await modelFile.exists()) {
        debugPrint('❌ Model file not found at: $modelPath');
        _initCompleter?.complete();
        return false;
      }
      
      final modelSize = await modelFile.length();
      debugPrint('✅ Model file confirmed: ${(modelSize / 1024 / 1024).toStringAsFixed(1)} MB');
      ...
      debugPrint('🔍 Looking for library file: $libraryFileName');
      ...
      if (libraryPath == null) {
        debugPrint('⚠️ Using system library path as fallback: $libraryFileName');
        libraryPath = libraryFileName;
      } else {
        debugPrint('✅ Found library at: $libraryPath');
      }

      // Set the library path
      debugPrint('🔗 Setting Llama.libraryPath to: $libraryPath');
      try {
        Llama.libraryPath = libraryPath;
        debugPrint('✅ Library path set successfully');
      } catch (e) {
        debugPrint('❌ Failed to set library path: $e');
        throw Exception('Failed to set library path: $e');
      }
      ...
      debugPrint('🚀 Creating LlamaParent with load command...');
      _llamaParent = LlamaParent(loadCommand);
      debugPrint('📡 Initializing LlamaParent isolate...');
      await _llamaParent!.init();
      debugPrint('✅ LlamaParent initialized successfully');
      ...
      debugPrint('📊 Service status: _isRunning = true');
      _isRunning = true;
      _initCompleter?.complete();

      return true;
    } catch (e) {
      debugPrint('💥 Error initializing LLM: $e');
      debugPrint('📊 Service status: _isRunning = false');
  ...
  /// 🔧 COMPATIBILITY: Initialize without parameters (for main.dart compatibility)
  /// This method auto-detects the model path and initializes the service
  Future<bool> initializeAuto() async {
    try {
      debugPrint('🤖 LlamaService: Starting auto-initialization...');
      
      // Try TinyLlama first (preferred for mobile)
      final tinyLlamaPath = await getModelPath(modelFileName: kTinyLlamaModelFileName);
      final tinyLlamaFile = File(tinyLlamaPath);
      
      debugPrint('🔍 Checking TinyLlama at: $tinyLlamaPath');
      final tinyLlamaExists = await tinyLlamaFile.exists();
      debugPrint('📁 TinyLlama file exists: $tinyLlamaExists');
      
      if (tinyLlamaExists) {
        final fileSize = await tinyLlamaFile.length();
        debugPrint('📏 TinyLlama file size: ${(fileSize / 1024 / 1024).toStringAsFixed(1)} MB');
        
        debugPrint('🚀 Attempting to initialize TinyLlama...');
        try {
          final success = await initialize(tinyLlamaPath);
          if (success) {
            debugPrint('✅ TinyLlama initialized successfully');
        return true;
            } else {
            debugPrint('❌ TinyLlama initialization returned false');
          }
        } catch (e) {
          debugPrint('💥 TinyLlama initialization threw exception: $e');
          // Continue to try fallback model
        }
      } else {
        debugPrint('⚠️ TinyLlama model not found at: $tinyLlamaPath');
      }
      
      // Fallback to standard Llama model
      final llamaPath = await getModelPath(modelFileName: kLlamaModelFileName);
      final llamaFile = File(llamaPath);
      
      debugPrint('🔍 Checking standard Llama at: $llamaPath');
      final llamaExists = await llamaFile.exists();
      debugPrint('📁 Standard Llama file exists: $llamaExists');
      
      if (llamaExists) {
        final fileSize = await llamaFile.length();
        debugPrint('📏 Standard Llama file size: ${(fileSize / 1024 / 1024).toStringAsFixed(1)} MB');
        
        debugPrint('🚀 Attempting to initialize standard Llama...');
        try {
          final success = await initialize(llamaPath);
          if (success) {
            debugPrint('✅ Standard Llama initialized successfully');
            return true;
          } else {
            debugPrint('❌ Standard Llama initialization returned false');
          }
        } catch (e) {
          debugPrint('💥 Standard Llama initialization threw exception: $e');
        }
      } else {
        debugPrint('⚠️ Standard Llama model not found at: $llamaPath');
      }
      
      // No models found or initialization failed
      final modelsDir = await _getModelsDirectory();
      debugPrint('❌ No LLM models could be initialized');
      debugPrint('📂 Models directory: $modelsDir');
      debugPrint('🔍 Expected files:');
      debugPrint('   - $kTinyLlamaModelFileName (preferred)');
      debugPrint('   - $kLlamaModelFileName (fallback)');
      ...
      try {
        ...
        if (await modelsDirectory.exists()) {
          ...
          debugPrint('📋 Files in models directory:');
          for (final file in files) {
            if (file is File) {
              final size = await file.length();
              debugPrint('   - ${path.basename(file.path)} (${(size / 1024 / 1024).toStringAsFixed(1)} MB)');
            }
          }
        } else {
        ...
        debugPrint('❌ Error listing models directory: $e');
      }
      
      return false;
    } catch (e) {
      debugPrint('💥 Auto-initialization failed with exception: $e');
  ...
  /// 🔧 COMPATIBILITY: Check if service is initialized (for recommendation_service.dart)
  bool get isInitialized => _isRunning;

  /// 🔧 COMPATIBILITY: Check if model is available (for recommendation_service.dart)
  Future<bool> isModelAvailable() async {
    try {
      final modelPath = await getModelPath(modelFileName: kTinyLlamaModelFileName);
      final modelFile = File(modelPath);
    ...
    /// 🔧 COMPATIBILITY: Generate explanation (for recommendation_service.dart)
  /// This bridges to the new optimized reasoning method
  Future<String> generateExplanation({
    ...
    return await generateReasoningExplanation(
        userQuery: userQuery,
  ...
  /// 🚀 SUPER PERFORMANT: Generate distilled search query for vector database
  /// Optimized for ~50-100ms response time with minimal tokens
  /// Converts user queries into semantic search terms for better vector matching
  Future<String> generateDistilledSearchQuery(String userQuery, {String? mediaType, String? context}) async {
    final stopwatch = Stopwatch()..start();
    
    if (!_isRunning) {
      stopwatch.stop();
      throw Exception('LLM not running - cannot generate distilled search query');
    }

    try {
      // Ultra-compact prompt optimized for speed
      final prompt = _buildDistillationPrompt(userQuery, mediaType, context);
      
      // Use fast generation with early stopping
      final response = await _generateFastResponse(
        prompt, 
        maxTokens: 20,        // Very short for search terms
        temperature: 0.3,     // Low temperature for focused results
        stopSequences: ['\n', '.', '|'],
      );
      
      stopwatch.stop();
      debugPrint('🚀 Search distillation completed in ${stopwatch.elapsedMilliseconds}ms');
      
      return _cleanDistilledSearch(response, userQuery);
    } catch (e) {
      stopwatch.stop();
      debugPrint('❌ Search distillation failed in ${stopwatch.elapsedMilliseconds}ms: $e');
      rethrow;
    }
  }

  /// 🧠 SUPER PERFORMANT: Generate reasoning explanation for suggestions
  /// Optimized for ~100-200ms response time with high-quality explanations
  /// Creates personalized explanations for why recommendations match user preferences
  Future<String> generateReasoningExplanation({
    ...
    if (!_isRunning) {
      stopwatch.stop();
      throw Exception('LLM not running - cannot generate reasoning explanation');
    }

    try {
      // Optimized prompt for reasoning generation
      final prompt = _buildReasoningPrompt(
        userQuery: userQuery,
        mediaTitle: mediaTitle,
        mediaType: mediaType,
        artist: artist,
      ...
      stopwatch.stop();
      debugPrint('🧠 Reasoning explanation completed in ${stopwatch.elapsedMilliseconds}ms');
      
      return _cleanReasoningResponse(response);
    } catch (e) {
      stopwatch.stop();
      debugPrint('❌ Reasoning explanation failed in ${stopwatch.elapsedMilliseconds}ms: $e');
      rethrow;
    }
  }

  /// Fast response generation optimized for mobile performance
  Future<String> _generateFastResponse(
    String prompt, {
    ...
    try {
      ...
      subscription = _responseStreamController?.stream.listen((token) {
        ...
        if (tokenCount >= maxTokens) {
          _llamaParent?.stop();
          if (!completer.isCompleted) {
            completer.complete(currentText.trim());
          }
          subscription?.cancel();
          timeoutTimer?.cancel();
          return;
        ...
        // Stop at sentence endings for natural completion
  ...
  /// Build ultra-compact prompt for search distillation
  String _buildDistillationPrompt(String userQuery, String? mediaType, String? context) {
    final buffer = StringBuffer();
    
    if (context != null && context.isNotEmpty) {
      // Focus on user preferences rather than the generic query
      if (context.contains('CONSTRAINTS:')) {
        final parts = context.split(' | CONSTRAINTS: ');
        buffer.write('Search terms for ${mediaType ?? "content"}: ${parts[0]}');
      } else {
        buffer.write('Search terms for ${mediaType ?? "content"}: $context');
      }
    } else {
      buffer.write('Extract key search terms from: "$userQuery"');
      if (mediaType != null) {
        buffer.write(' for $mediaType');
      }
    }
  ...
  String _cleanDistilledSearch(String response, String originalQuery) {
    String cleaned = response
        .replaceAll(RegExp(r'[^\w\s,]'), '') // Remove special chars except commas
        .trim();
    ...
    // Limit to essential terms, prioritize meaningful words
    final terms = cleaned.split(RegExp(r'[,\s]+'))
        .where((term) => term.length > 2)
        .where((term) => !['for', 'the', 'and', 'music', 'content'].contains(term.toLowerCase()))
        .take(5)
        .join(' ');
