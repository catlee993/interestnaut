...
import '../generated/recommendation.pbgrpc.dart' as grpc;
...
class GrpcRecommendationClient {
  ...
  late grpc.RecommendationServiceClient _client;
  ...
  Future<void> init() async {
    try {
      ...
      _client = grpc.RecommendationServiceClient(_channel);
      _initialized = true;
  ...
  Future<Map<String, dynamic>> healthCheck() async {
    ...
    try {
      final request = grpc.HealthCheckRequest();
  ...
  Future<List<MediaSuggestion>> getRecommendations({
    ...
    try {
      final request = grpc.RecommendationRequest()
        ..mediaType = mediaType
        ..userQuery = userQuery ?? ''
  ...
  Future<List<MediaSuggestion>> searchMedia({
    ...
    try {
      final request = grpc.SearchRequest()
        ..mediaType = mediaType
        ..constraints.addAll(constraints)
        ..excludeMediaIds.addAll(excludeMediaIds)
        ..limit = limit
  ...
  Future<List<MediaSuggestion>> getSimilarItems({
    ...
    try {
      final request = grpc.SimilarItemsRequest()
  ...
  Future<List<MediaSuggestion>> getRandomMedia({
    ...
    try {
      final request = grpc.RandomMediaRequest()
        ..mediaType = mediaType
        ..excludeMediaIds.addAll(excludeMediaIds)
        ..limit = limit;
  ...
  Future<Map<String, dynamic>> searchThemes({
    ...
    try {
      final request = grpc.SearchThemesRequest()
        ..mediaType = mediaType
        ..query = query
        ..limit = limit;
  ...
  Future<Map<String, dynamic>> searchGenres({
    ...
    try {
      final request = grpc.SearchGenresRequest()
        ..mediaType = mediaType
        ..query = query
        ..limit = limit;
  ...
  Future<MediaSuggestion?> getMediaDetails({
    ...
    try {
      final request = grpc.MediaDetailsRequest()
        ..mediaId = mediaId
        ..mediaType = mediaType;
  ...
  MediaSuggestion _convertToMediaSuggestion(grpc.MediaItem item, String mediaType) {
